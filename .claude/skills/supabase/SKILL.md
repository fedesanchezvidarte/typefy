...
- **API key and client exposure**
   - **Never expose the `service_role` or secret key in public clients.** Prefer publishable keys for frontend code. Legacy `anon` keys are only for compatibility. In SvelteKit, any `PUBLIC_` env var (`$env/static/public` / `$env/dynamic/public`) is sent to the browser; keep secrets in `$env/static/private`.
